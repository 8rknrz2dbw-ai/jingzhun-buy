...
> 本 App 與農夫版「**精準務農**」是同一份程式碼、由 `index.html` 頂端的
> `const APP="merchant"` 切換而來。兩支各自獨立部署、各自一個 App。
...
* 註：`scripts/` 內的 `build_advisory.py`、`notify.py`、`fetch_yield.py` 與
  `harvest_advisory.json`、`data/` 是農夫版（精準務農）留下的，商人版**用不到**，可以留著不管、也可以刪掉。
